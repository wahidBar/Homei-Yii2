...
trait ProviderTrait
{
    ...
    protected function shorthandAttributeFormat($attribute, $model)
    {
        ...
        if ($column->phpType === 'boolean' || $column->name == "flag") {
            $format = 'boolean';
            ...
            $format = 'url';
        } elseif (strposa($column->name, ['gambar', 'foto', 'image', 'photo'])) {
            $format = 'myImage';
        } elseif (strposa($column->name, ['_at', 'tanggal', 'date', 'tgl_', '_pada', 'time'])) {
            $format = 'iddate';
        } elseif (strposa($column->name, ['price', 'harga', 'biaya'])) {
            $format = 'rp';
        } else {
            $format = 'text';
        }
