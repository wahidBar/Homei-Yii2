...
class SidebarMenu extends Widget
{
    public static function getMenu($roleId, $parentId=NULL){
        $output = [];
        foreach(Menu::find()->where(["parent_id"=>$parentId])->orderBy(['order' => SORT_ASC])->all() as $menu){
            $obj = [
                "label" => $menu->name,
                "icon" => $menu->icon,
    ...
    private static function roleHasAccess($roleId, $menuId){
        $roleMenu = \app\models\RoleMenu::find()->where(["menu_id"=>$menuId, "role_id"=>$roleId])->all();
        if($roleMenu){
            return TRUE;
        }else{
            return FALSE;
