...
class BarangMasukExport extends ExcelHelper
{
    public $models;
    ...
    function styleHeader(&$sheet)
    {
        ...
        $sheet->getCell('B1')->setValue('Gudang');
        ...
        $sheet->getCell('C1')->setValue('Nama Barang');
        ...
        $sheet->getCell('D1')->setValue('No. PO');
        ...
        $sheet->getCell('E1')->setValue('Jumlah');
        ...
        $sheet->getStyle('F1')->applyFromArray(array_merge($this->font(true, '11', 'Times New Roman'), $this->alignmentCenter()));
        $sheet->getCell('F1')->setValue('Tanggal');
        $sheet->getStyle("F1")->applyFromArray($this->border('bottom', PHPExcel_Style_Border::BORDER_MEDIUM, '000000'));
        $sheet->getStyle("F1")->applyFromArray($this->border('right', PHPExcel_Style_Border::BORDER_MEDIUM, '000000'));
    }
    ...
    function generateData($models, &$objPHPExcel, &$sheet, &$row = 2, &$no = 1, &$abjad = "A")
    {
        foreach ($models as $key => $model) {
            $sheet->getStyle('A' . $row)->applyFromArray($this->font(true, '11', 'Times New Roman'));
                ...
                $sheet->getCell('B' . $row)->setValue($model->masterGudang->nama);
                ...
                $sheet->getCell('C' . $row)->setValue($model->supplierBarang->nama_barang);
                ...
                $sheet->getCell('D' . $row)->setValue($model->no_po);
                ...
                $sheet->getCell('E' . $row)->setValue($model->jumlah);
                ...
                $sheet->getCell('F' . $row)->setValue($model->created_at);
                $sheet->getStyle('F' . $row)->applyFromArray($this->border('bottom', PHPExcel_Style_Border::BORDER_THIN, '000000'));
                $sheet->getStyle('F' . $row)->applyFromArray($this->border('right', PHPExcel_Style_Border::BORDER_THIN, '000000'));
                $no++;
                $row++;
        }
