 ...
 **/
class PageController extends \app\components\productive\DefaultActiveController
{
    ...
    public function actionCreate()
    {
        ...
        try {
            if ($model->load($_POST)) :
                $model->slug = substr(Inflector::slug($model->title), 0, 200) . time();

                $instance = UploadedFile::getInstance($model, 'thumbnail');
                $response = $this->uploadFile($instance, 'thumbnail');
                if ($response->success == false) :
                    toastError($response->message);
                    goto end;
                else :
                    $model->thumbnail = $response->filename;
                endif;
